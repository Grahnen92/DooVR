...
void modifyMesh(Mesh* mesh, GLFWwindow* window, Device* wand) {
	...
	if (glfwGetKey(window, GLFW_KEY_O)) {
		...
		mesh->updateVertexArray(wand->getAnalogPosition()[0], wand->getAnalogPosition()[1], true);
		//			deform.dilate(mTest.getVertexList(), mTest.getIndexList);
	}
	if (glfwGetKey(window, GLFW_KEY_P)) {
		mesh->updateVertexArray(wand->getAnalogPosition()[0], wand->getAnalogPosition()[1], false);
		//			deform.dilate(mTest.getVertexList(), mTest.getIndexList);
	}

...
int runOvr()
{
	...
	*/
	...
	//glfwWindowHint(GLFW_EXPOSE_NATIVE_GLX , GLFW_CONTEXT_VERSION_MAJOR);
...
//			glUniformMatrix4fv(locationMV, 1, GL_FALSE, MVstack.getCurrentMatrix());
			...
			//glLoadIdentity();
			//glMultMatrixf(&(g_ProjectionMatrix[l_Eye].Transposed().M[0][0]));
			...
			//	glMultMatrixf(&(l_ModelViewMatrix.Transposed().M[0][0]));
				...
				MVstack.translate(glm::vec3(-g_EyePoses[l_Eye].Position.x, -g_EyePoses[l_Eye].Position.y, -g_EyePoses[l_Eye].Position.z));

				cout << -g_EyePoses[l_Eye].Position.x << endl;
				...
				//MVstack.translate(glm::vec3(g_CameraPosition.x, g_CameraPosition.y, g_CameraPosition.z));

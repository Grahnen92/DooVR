...
void moveEntity(Device * wand, vector < Entity * > *objectList, floatwandRadius);
...
int Oculus::runOvr() {
    ...
    g_RenderTargetSize.h = (l_EyeTextureSizes[ovrEye_Left].h > l_EyeTextureSizes[ovrEye_Right].h
                            ? l_EyeTextureSizes[ovrEye_Left].h : l_EyeTextureSizes[ovrEye_Right].h);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, g_RenderTargetSize.w, g_RenderTargetSize.h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 0);
    // Linear filtering...
    ...
    ovrBool l_ConfigureResult = ovrHmd_ConfigureRendering(hmd, &g_Cfg.Config, G_DISTORTIONCAPS, hmd->MaxEyeFov,
                                                          g_EyeRenderDesc);
    ...
    uint32_t l_SupportedSensorCaps =
    ...
    while (!glfwWindowShouldClose(l_Window)) {
        ...
        for (int l_EyeIndex = 0; l_EyeIndex < ovrEye_Count; l_EyeIndex++) {
            ...
            float eyePoses[3] = {-g_EyePoses[l_Eye].Position.x, -g_EyePoses[l_Eye].Position.y,
                                 -g_EyePoses[l_Eye].Position.z};
